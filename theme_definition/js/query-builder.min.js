 ...
 */
 ...
 /*
  *	Some modification have been made by Manuel Dupont
  * for Eptica (http://www.eptica.com) while working on a prototype.
  */
...
(function($){
    ...
    QueryBuilder.prototype.init = function(options) {
        ...
        this.$el.on('change.queryBuilder', '.rule-filter-container select[name$=_filter]', function() {
            ...
            that.createRuleOperators($rule);
			that.createRuleInput($rule);
        });
        ...
        return (function parse($group) {
            ...
            for (var i=0, l=$elements.length; i<l; i++) {
                ...
                if ($rule.hasClass('rule-container')) {
					/* ADDED MDU: replaced filterId by our only filter id: "descriptor" */
                    var filter = that.getFilterById('descriptor'),
    ...
    QueryBuilder.prototype.createRuleOperators = function($rule) {
		...
		/* ADDED MDU: replaced filterId by our only filter id: "descriptor" */
        var operators = this.getOperators('descriptor'),
    ...
    QueryBuilder.prototype.createRuleInput = function($rule) {
		...
		/* ADDED MDU: replaced filterId by our only filter id: "descriptor" */
        var filter = this.getFilterById('descriptor'),
    ...
    QueryBuilder.prototype.validateValue = function($rule, value, filter, operator) {
		...
		/* ADDED MDU: add possibility to disable emptiness check for a filter:
					  if filter.allow_empty_value is defined and false
		*/
		var check_empty_value = filter.allow_empty_value !== undefined && !filter.allow_empty_value;
		
        switch (filter.input) {
            case 'radio':
                if (check_empty_value && value === undefined) {
                    return 'radio_empty';
                }
                break;

            case 'checkbox':
                if (check_empty_value && value.length === 0) {
            ...
            case 'text': default:
                switch (filter.internalType) {
                    case 'string':
                        ...
                        else if (check_empty_value && value.length === 0) {
                            return 'string_empty';
                        }
    ...
    QueryBuilder.prototype.initSortable = function() {
        ...
        this.$el.on('mousedown', '.drag-handle', function(e) {
            isHandle = true;
        }).on('mouseup', '.drag-handle', function(e) {
            isHandle = false;
        ...
        }).on('dragenter', '[draggable]', function(e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    QueryBuilder.prototype.getRuleFilter = function($rule) {
		/* ADDED MDU: we need only one filter : "descriptor" */
        return 'descriptor';
    };
